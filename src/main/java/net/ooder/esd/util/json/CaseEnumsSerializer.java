...
public class CaseEnumsSerializer<T extends Enumstype> implements ObjectSerializer, ObjectDeserializer {
    public static final JSONEditorSerializer instance = new JSONEditorSerializer();

    public CaseEnumsSerializer() {
        super();
    }
            ...
            if (enumClass.isEnum()) {
                try {
                    ...
                    for (T enumstype : enums) {
                        Enumstype etype = (Enumstype) enumstype;
                        if (etype.getType().toUpperCase().equals(json.toUpperCase())) {
                            return enumstype;
                        }
                    }
