...
public class ContentBlockPageUtil {

    public static <T extends ContentBlockItem, K> TreeListResultModel<List<T>> getTabList(Set<K> objs, Class<T> clazz) {
        List<K> result = new ArrayList<K>();
        result.addAll(objs);
        return getTabList(result, clazz);
    }

    public static <T extends ContentBlockItem, K> TreeListResultModel<List<T>> getTabList(List<K> objs, Class<T> clazz) {
    ...
    static ContentBlockItemBean getContentBlockItem(CustomContentBlockViewBean viewBean, Class sourceClazz, Object obj) {
        ContentBlockItemBean contentBlockItemBean = null;
        Class treeClass = obj.getClass();
        if (obj instanceof LineListItem) {
            return viewBean.getChildItemBean((TabItem) obj);
        }

        List<ContentBlockItemBean> childTabs = viewBean.getContentBlockItemBeans();
        for (ContentBlockItemBean child : childTabs) {
            try {
        ...
        for (Constructor constructor : constructors) {
            ...
            if (paramClass.length > 0 && paramClass[0].isAssignableFrom(treeClass)) {
                try {
                    contentBlockItemBean = new ContentBlockItemBean(constructor, viewBean);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    static <T extends ContentBlockItem, K extends IconEnumstype> List<T> fillObjs(List<K> objs, Class<T> clazz) {
    ...
    static <T extends ContentBlockItem, K> ContentBlockItem fillObj(K obj, Class<T> clazz) {
        ...
        T t = null;
        ContentBlockItemBean tabItemBean = getContentBlockItem(viewBean, clazz, obj);
        if (tabItemBean != null) {
            try {
    ...
    public static <T extends ContentBlockItem, K extends IconEnumstype> TreeListResultModel<List<T>> changTabList(ListResultModel<List<K>> objs, Class<T> clazz) {
        TreeListResultModel<List<T>> userStatusInfo = new TreeListResultModel<List<T>>();
        try {
            List<T> pageResult = new ArrayList<T>();
    ...
    public static <T extends ContentBlockItem, K extends IconEnumstype> TreeListResultModel<List<T>> getDefaultDataList(ListResultModel<List<K>> objs, Class<T> clazz) {
        TreeListResultModel<List<T>> userStatusInfo = new TreeListResultModel<List<T>>();
        try {
            List<T> pageResult = new ArrayList<T>();
            List<K> list = objs.get();
