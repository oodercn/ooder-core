...
import net.ooder.esd.bean.field.CustomContentBlockFieldBean;
...
public class CustomFieldContentBlockComponent extends ContentBlockComponent {
    public static final Log logger = LogFactory.getLog(JDSConstants.CONFIG_KEY, CustomFieldContentBlockComponent.class);
    ...
    public CustomFieldContentBlockComponent(EUModule euModule, FieldFormConfig<CustomContentBlockFieldBean, ?> field, String target, Object value, Map valueMap) {
        ...
        if (contentBlockViewBean == null) {
            contentBlockViewBean = new CustomContentBlockViewBean(field.getMethodConfig());
        }
        //合并方法注解
        OgnlUtil.setProperties(JSON.parseObject(JSON.toJSONString(fieldBean), Map.class), contentBlockViewBean, false, false);
        ContentBlockProperties contentBlockProperties = new ContentBlockProperties(contentBlockViewBean);
        contentBlockProperties.setId(field.getId());
        contentBlockProperties.setName(field.getFieldname());
        contentBlockProperties.setValue(value);
        ...
        if (fieldBean.getListMenuBean() != null) {
            ...
            for (String key : keset) {
                ...
                contentBlockProperties.addItem(new ContentBlockItem(item));
            }
            euModule.getComponent().addChildren(this.getApis().toArray(new APICallerComponent[]{}));
        }

        this.setProperties(contentBlockProperties);
        this.setTarget(target);

    }


    private void lazyLoad(EUModule euModule, FieldFormConfig field, CustomFieldContentBlockComponent listComponent, Class bindClass) {
        try {
            ...
            if (loadMethod != null) {
                ...
                UrlPathData galleryData = new UrlPathData(listComponent.getAlias(), ResponsePathTypeEnum.GALLERY, "data");
    ...
    public APICallerComponent addModuleMenu(String className, APICallerComponent component, Map<String, Object> paramsMap) {
        ...
        if (expression == null || expression.equals("") || this.parExpression(expression)) {
            ...
            if (customItems.size() > 0) {
                ...
                ShowPageAction action = new ShowPageAction(ContentBlockEventEnum.onClick);
            ...
            if (items != null && items.size() > 0) {
                for (CustomMenuItem item : items) {
                    ...
                    if (actions != null && actions.size() > 0) {
                        ...
                        Action action = new Action(ContentBlockEventEnum.onClick);
        ...
        for (CustomAction actionType : actionTypes) {
            ...
            try {
                if (exprossion != null && !exprossion.equals("") && EsbUtil.parExpression(exprossion, Boolean.class)) {
                    CustomConditionAction action = new CustomConditionAction(actionType, type,ContentBlockEventEnum.onClick);
                    this.addAction( action);
                    actions.add(action);
                }
            } catch (Throwable e) {
