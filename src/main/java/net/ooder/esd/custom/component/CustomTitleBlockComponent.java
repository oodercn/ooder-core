...
import net.ooder.esd.bean.data.CustomTitleBlockDataBean;
...
public class CustomTitleBlockComponent extends CustomModuleComponent<TitleBlockComponent> {
    ...
    public CustomTitleBlockComponent() {
        super();
    }

    public CustomTitleBlockComponent(EUModule module, MethodConfig methodConfig, Map<String, Object> valueMap) {
        super(module, methodConfig, valueMap);

    }

    public void addChildNav(TitleBlockComponent currComponent) {
    ...
    protected void fillTitleBlockAction(CustomTitleBlockViewBean viewBean, Component currComponent) {
        ...
        for (CustomTitleBlockEvent eventType : customFormEvents) {
            for (CustomAction actionType : eventType.getActions(false)) {
                ...
                if (methodConfig != null) {
                    if (!methodConfig.isModule()) {
                        ...
                        action = new MShowPageAction(methodConfig, TitleBlockEventEnum.onMoreClick);
        ...
        List<TitleBlockMenu> customFormMenus = viewBean.getCustomMenu();
        if (customFormMenus != null && customFormMenus.size() > 0) {
            this.getMenuBar(viewBean.getMenuBar()).addMenu(customFormMenus.toArray(new TitleBlockMenu[]{}));
        }

        List<TitleBlockMenu> customBottombar = viewBean.getBottombarMenu();
        if (customBottombar != null && customBottombar.size() > 0) {
            this.getBottomBar(viewBean.getBottomBar()).addMenu(customBottombar.toArray(new TitleBlockMenu[]{}));
        }
        super.fillAction(viewBean);
        this.fillMenuAction(viewBean, currComponent);
    }

    protected void fillMenuAction(CustomTitleBlockViewBean viewBean, Component currComponent) {
        try {
            if (viewBean != null) {
    ...
    protected void fillViewAction(MethodConfig methodConfig) {
        CustomTitleBlockDataBean dataBean = (CustomTitleBlockDataBean) methodConfig.getDataBean();
        if (dataBean != null) {
...
//            }
    ...
    APICallerComponent[] genAPIComponent(MethodConfig methodConfig, Component ctxComponent) throws JDSException, ClassNotFoundException {
        TitleBlockComponent currComponent = this.getCurrComponent();
        ...
        if (deleteMethodBean != null) {
            UrlPathData requestPathData = new UrlPathData(currComponent.getAlias(), RequestPathTypeEnum.TITLEBLOCK, "");
        ...
        if (clickFlagPath != null && !clickFlagPath.equals("")) {
            ...
            clickFlagMethodBean = methodConfig.getDataBean().getMethodEvent(CustomTitleBlockEvent.ONMORECLICK);
        }
        ...
        if (clickFlagMethodBean != null) {
            ...
            if (!clickFlagMethodBean.isModule()) {
                ...
                currComponent.addAction(new CustomAPICallAction(flagClickApi, TitleBlockEventEnum.onMoreClick));

            } else {
                ShowPageAction action = new ShowPageAction(TitleBlockEventEnum.onMoreClick);
        ...
        if (methodBean != null) {
            UrlPathData responsePathData = new UrlPathData(currComponent.getAlias(), ResponsePathTypeEnum.TITLEBLOCK, "data");
