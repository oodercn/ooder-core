...
import net.ooder.esd.custom.component.CustomMFormComponent;
...
public class FullMFormComponent extends CustomModuleComponent<CustomMFormComponent> {
    ...
    public FullMFormComponent(EUModule euModule, MethodConfig methodConfig, Map valueMap) {
    ...
    void init(MethodConfig methodConfig) throws JDSException {
        ...
        CustomMFormComponent currComponent =  new CustomMFormComponent(euModule, viewBean.getAllFields(), valueMap, viewBean);
            ...
            if (serviceObjs != null) {
                for (Class obj : serviceObjs) {
                    if (!obj.equals(Void.class)) {
                        CustomToolsBar bar = (CustomToolsBar) PluginsFactory.getInstance().initMenuClass(obj,CustomToolsBar.class);
