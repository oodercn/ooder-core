...
import net.ooder.esd.bean.data.CustomOpinionDataBean;
...
public class CustomOpinionComponent extends CustomModuleComponent<OpinionComponent> {
    ...
    public CustomOpinionComponent() {
        super();
    }

    public CustomOpinionComponent(EUModule module, MethodConfig methodConfig, Map<String, Object> valueMap) throws ClassNotFoundException {
        super(module, methodConfig, valueMap);

    }

    public void addChildNav(OpinionComponent currComponent) {
    ...
    protected void fillOpinionAction(CustomOpinionViewBean viewBean, Component currComponent) {
        ...
        for (CustomOpinionEvent eventType : customFormEvents) {
        ...
        List<OpinionMenu> customFormMenus = viewBean.getCustomMenu();
        if (customFormMenus != null && customFormMenus.size() > 0) {
            this.getMenuBar(viewBean.getMenuBar()).addMenu(customFormMenus.toArray(new OpinionMenu[]{}));
        }

        List<OpinionMenu> customBottombar = viewBean.getBottombarMenu();
        if (customBottombar != null && customBottombar.size() > 0) {
            this.getBottomBar(viewBean.getBottomBar()).addMenu(customBottombar.toArray(new OpinionMenu[]{}));
        }
        super.fillAction(viewBean);
        this.fillMenuAction(viewBean, currComponent);
    }

    protected void fillMenuAction(CustomOpinionViewBean viewBean, Component currComponent) {
        try {
            if (viewBean != null) {
    ...
    protected void fillViewAction(MethodConfig methodConfig) {
        CustomOpinionDataBean dataBean = (CustomOpinionDataBean) methodConfig.getDataBean();
    ...
    APICallerComponent[] genAPIComponent(MethodConfig methodConfig, Component ctxComponent) throws JDSException, ClassNotFoundException {
        OpinionComponent currComponent = this.getCurrComponent();
        ...
        if (dataUrl != null && !dataUrl.equals("")) {
            ...
            if (methodBean != null) {
                UrlPathData responsePathData = new UrlPathData(currComponent.getAlias(), ResponsePathTypeEnum.OPINION, "data");
