...
public class CustomFieldGalleryComponent extends GalleryComponent {
    ...
    public CustomFieldGalleryComponent(EUModule euModule, FieldFormConfig<CustomGalleryFieldBean, ?> field, String target, Object value, Map valueMap) {
        ...
        CustomGalleryViewBean galleryViewBean = (CustomGalleryViewBean) field.getMethodConfig().getView();
        if (galleryViewBean == null) {
            galleryViewBean = new CustomGalleryViewBean(field.getMethodConfig().getViewClass().getCtClass());
        }
        ...
        //合并方法注解
        OgnlUtil.setProperties(JSON.parseObject(JSON.toJSONString(fieldBean), Map.class), galleryViewBean, false, false);
        GalleryProperties galleryProperties = new GalleryProperties(galleryViewBean);
        galleryProperties.setId(field.getId());
        galleryProperties.setName(field.getFieldname());
        galleryProperties.setValue(value);
        if (customListBean != null) {
            if (customListBean.getDynLoad() != null && customListBean.getDynLoad() && customListBean.getBindClass() != null) {
                lazyLoad(euModule, field, this, customListBean.getBindClass());
                galleryViewBean.setBindService(customListBean.getBindClass());
            }
        ...
        if (fieldBean.getListMenuBean() != null) {
            ...
            for (String key : keset) {
                ...
                galleryProperties.addItem(new GalleryItem(item));
            }
            euModule.getComponent().addChildren(this.getApis().toArray(new APICallerComponent[]{}));
        }

        this.setProperties(galleryProperties);
        this.setTarget(target);

    }


    private void lazyLoad(EUModule euModule, FieldFormConfig field, CustomFieldGalleryComponent listComponent, Class bindClass) {
        try {
