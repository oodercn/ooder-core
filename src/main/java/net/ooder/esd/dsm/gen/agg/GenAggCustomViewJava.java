...
public class GenAggCustomViewJava extends BaseAggCallabel {
    ...
    public List<JavaSrcBean> call() throws Exception {
        ...
        String baseUrl = domainInst.getEuPackage();
        ...
        for (JavaTemp javatemp : viewTemps) {
            if (javatemp.getRangeType() != null && javatemp.getViewType().equals(moduleViewType.getDefaultView())) {
                ...
                if (canGen) {
                    ...
                    if (!packageName.endsWith(moduleName) && !moduleName.equals("") && !moduleName.toLowerCase().equals(simClassName.toLowerCase())) {
                        packageName = packageName + "." + moduleName.toLowerCase();
                    }

                    if (!moduleName.toLowerCase().endsWith("." + simClassName.toLowerCase()) && javatemp.getNamePostfix().equals("**")) {
