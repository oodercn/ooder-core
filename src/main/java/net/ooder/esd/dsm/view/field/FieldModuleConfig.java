...
public class FieldModuleConfig implements ESDFieldConfig, Comparable<FieldModuleConfig> {
    String id;
    ...
    public FieldModuleConfig() {

    }


    public FieldModuleConfig(CustomModuleBean moduleBean) {
        this.uid = false;
        this.pid = false;
        this.serialize = true;
        this.colHidden = false;
        this.sourceClassName = moduleBean.getSourceClassName();
        this.fieldname = moduleBean.getName();
        this.methodName = moduleBean.getMethodName();
    ...
    public FieldModuleConfig(CustomViewBean customViewBean) {
        if (customViewBean.getMethodConfig()!=null){
            init(customViewBean.getMethodConfig());
        }else if (customViewBean.getModuleBean()!=null) {
            init(customViewBean.getModuleBean());
        }

    }

    public FieldModuleConfig(MethodConfig methodConfig) {
        init(methodConfig);
    }
    ...
    public FieldModuleConfig(FieldFormConfig esdField) {
        MethodConfig fieldMethodConfig = null;
        try {
            this.sourceMethodName = esdField.getSourceMethodName();
            this.sourceClassName = esdField.getSourceClassName();
            this.entityClassName = esdField.getEntityClassName();
            this.customBean=esdField.getCustomBean();


            AggregationManager aggregationManager = DSMFactory.getInstance().getAggregationManager();
            ApiClassConfig apiClassConfig = aggregationManager.getApiClassConfig(esdField.getViewClassName());
    ...
    public EUModule getModule(Map valueMap) throws JDSException {
        ...
        String projectVersionName =DSMFactory.getInstance().getDefaultProjectName();
        EUModule module = null;
        if (euClassName != null && !euClassName.equals("")) {
            module = client.getModule(euClassName, projectVersionName);
            module.getComponent().setAlias(this.id + "Panel");
        } else {
            module = client.getCustomModule(this.getUrl(), projectVersionName, valueMap);
        }
        if (module != null) {
    ...
    public MethodConfig getMethodConfig() {
        if (methodConfig == null && this.getViewClassName() != null) {

            try {
                AggregationManager aggregationManager = DSMFactory.getInstance().getAggregationManager();

                //优先从当前应用获取
                AggEntityConfig aggEntityConfig = aggregationManager.getAggEntityConfig(this.getViewClassName(),false);
                if (aggEntityConfig != null) {
                    methodConfig = aggEntityConfig.getMethodByName(methodName == null ? fieldname : methodName);
                }
                //外部绑定从Source获取
                if (methodConfig == null) {
                    ApiClassConfig apiClassConfig = aggregationManager.getApiClassConfig(this.getSourceClassName());
...
//    }
    ...
    @Override
    public int compareTo(FieldModuleConfig o) {
