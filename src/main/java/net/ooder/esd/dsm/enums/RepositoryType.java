...
public enum RepositoryType implements IconEnumstype {
    ...
    REPOSITORY("仓储接口(CRUD)", "spafont spa-icon-c-webapi", 2, UserSpace.CRUD),
    REPOSITORYIMPL("仓储实现(CRUD)", "spafont spa-icon-tools", 3, UserSpace.CRUD),

    BPMBEAN("表单实体(BPM)", "spafont spa-icon-conf", 0, UserSpace.FORM),
    BPMRSERVICE("流程存储服务(BPM)", "spafont spa-icon-c-webapi", 1, UserSpace.FORM),

    USERBEAN("通用实体(User)", "spafont spa-icon-conf", 0, UserSpace.USER),
    USERSERVICE("存储接口(User)", "spafont spa-icon-c-webapi", 1, UserSpace.USER),

    VIEWBEAN("实体(View)", "spafont spa-icon-conf", 0, UserSpace.VIEW),
    VIEWSERVICE("服务(View)", "spafont spa-icon-c-webapi", 1, UserSpace.VIEW);
